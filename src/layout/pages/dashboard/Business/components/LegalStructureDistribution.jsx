...
import { getLegalStructure } from "../../../../../functions/energyAndBusinessAndPersonalAndForestryAndMobility"; // Adjust path as needed
...
const LegalStructureDistributionChart = () => {
  const [chartOptions, setChartOptions] = useState({});
  const [totalStructures, setTotalStructures] = useState(0); // State for central text
  ...
  const {
    ...
    queryKey: ["legal-structure-distribution-data", villageName, countryName],
    queryFn: () => getLegalStructure(countryName, villageName), // Adjust API call args if needed
    enabled: !!countryName, // Enable query if countryName is available
  });

  useEffect(() => {
    if (
      legalStructureResponse?.data &&
      Array.isArray(legalStructureResponse.data) &&
      legalStructureResponse.data.length > 0
    ) {
      // Sort data by count descending for better visualization
      const sortedData = [...legalStructureResponse.data].sort(
        (a, b) => (b.count || 0) - (a.count || 0)
      );

      const pieData = sortedData.map(item => ({
        name: item.legalStructure?.en || "Unknown Legal Structure", // Use English name for display
        y: typeof item.count === 'number' ? item.count : 0,
      }));

      const calculatedTotal = pieData.reduce((sum, item) => sum + item.y, 0);
      setTotalStructures(calculatedTotal); // Set total for central display
      ...
      setChartOptions({
        ...
        title: {
          text: `Business Legal Structure Distribution`,
          align: "left",
          style: {
            fontSize: "18px",
            fontWeight: "bold",
        ...
        lang: {
            noData: "No legal structure distribution data available."
        },
        noData: {
            style: {
                fontWeight: 'bold',
      ...
      setTotalStructures(0); // Reset total if no data
    }
  }, [legalStructureResponse, villageName, countryName]);
  ...
  const getNoDataOptions = () => ({
    ...
    lang: { noData: "No legal structure distribution data available." },
    ...
    return (
      <StyledCard sx={{ marginTop: 4, paddingY: "72px" }}>
        <div style={{ color: "red", padding: 16, textAlign: "center" }}>
          Error loading legal structure data: {error?.message || "Unknown error."}
        </div>
      </StyledCard>
    );
  }

  const hasValidDataForDisplay = legalStructureResponse?.data &&
                                legalStructureResponse.data.some(item => typeof item.count === 'number' && item.count > 0);

  if (!hasValidDataForDisplay) {
    return (
      <StyledCard sx={{ marginTop: 4, paddingY: "72px" }}>
        <div style={{ padding: 24, textAlign: "center" }}>
          No legal structure distribution data available.
        </div>
      </StyledCard>
    );
  }
    ...
    <StyledCard sx={{ marginTop: 4, paddingTop: "72px" }}>
      <CardContent sx={{ position: 'relative' }}> {/* Add relative position to CardContent */}
        ...
        <Typography
          ...
          sx={{ marginTop: 2 }}
        >
          Distribution of businesses by their legal structure.
        </Typography>
      </CardContent>
    </StyledCard>
  );
...
export default LegalStructureDistributionChart;