...
import { fetchGrains, fetchTea } from "../../../functions/consumption";
...
function TeaCoffee() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    select: (data) => {
      ...
      return { jsonData: data, tea_coffee: arr };
    },
  });
  ...
  const rows = data?.tea_coffee?.map((_cultivation, index) => ({
    id: index + 1,
          ...
          <div>
            <i
              ...
              onClick={(e) => {
                ...
                // selectData(tea_coffee[params.row.id]);
              }}
              id={params.row.id}
            ></i>
            <Menu
