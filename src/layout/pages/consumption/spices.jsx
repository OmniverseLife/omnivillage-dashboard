...
import { fetchGrains, fetchSauce } from "../../../functions/consumption";
...
function Spices() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    select: (data) => {
      ...
      return { jsonData: data, spices: arr };
    },
  });
  ...
  const rows = data?.spices?.map((_cultivation, index) => ({
    id: index + 1,
          ...
          <div>
            <i
              ...
              onClick={(e) => {
                ...
                // selectData(spices[params.row.id]);
              }}
              id={params.row.id}
            ></i>
            <Menu
