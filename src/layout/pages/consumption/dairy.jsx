...
import { fetchDairy, fetchGrains } from "../../../functions/consumption";
...
function Dairy() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    select: (data) => {
      ...
      return { jsonData: data, dairy: arr };
    },
  });
  ...
  const rows = data?.dairy?.map((_cultivation, index) => ({
    id: index + 1,
          ...
          <div>
            <i
              ...
              onClick={(e) => {
                ...
                // selectData(dairy[params.row.id]);
              }}
              id={params.row.id}
            ></i>
            <Menu
