...
import { fetchGrains, fetchHerbs } from "../../../functions/consumption";
...
function Herbs() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    select: (data) => {
      ...
      return { jsonData: data, herbs: arr };
    },
  });
  ...
  const rows = data?.herbs?.map((_cultivation, index) => ({
    id: index + 1,
          ...
          <div>
            <i
              ...
              onClick={(e) => {
                ...
                // selectData(herbs[params.row.id]);
              }}
              id={params.row.id}
            ></i>
            <Menu
